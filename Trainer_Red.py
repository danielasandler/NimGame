...
def main ():
    ...
    player2 = Random_Agent(player=1,env=env)   #0.1 train=True, random=0- זה היה בתוך הסוגריים
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optim,[20*10000, 20*20000, 20*30000, 20*50000], gamma=0.5)
    
    for epoch in range(epochs):
        print(f'epoch = {epoch}', end='\r')
        state_1 = env.get_init_state((ROWS,COLS))
        action_2 = player2.get_Action(state=state_1, env=env)
        ...
        while not env.is_over(state_1):
            ...
            if loss_count <= 1000:
                ...
                avgLoss += (loss.item()-avgLoss)* 0.001 
        ...
        if (epoch+1) % 100 == 0:
            ...
            if best_res < res:      
                best_res = res
                # if best_res > 32 and tester_fix(1) == (1,0): 
